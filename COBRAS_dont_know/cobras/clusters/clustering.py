...
class Clustering:
    ...
    def get_all_superinstances(self):
        result = []
        for cluster in self.clusters:
            result.extend(cluster.super_instances)
        return result

    def construct_superinstances(self):
        superinstances = []
        for cluster in self.clusters:
            superinstances.extend(cluster.get_all_points_per_superinstance())

        pred = [-1] * sum([len(x) for x in superinstances])
        for i, pts in enumerate(superinstances):
            for pt in pts:
                pred[pt] = i
        return pred
