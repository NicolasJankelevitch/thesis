...
from test_cobras.evaluate_clustering.evaluation.aligned_rank import calculate_aligned_rank


def read_average_aris(algo_name):
    with open(os.path.join(FOLDS_RESULT_PATH,algo_name, "ari_averages.txt"), mode = 'r') as file:
...
def calculate_and_write_aligned_rank(algo_names, comparison_name, dataset_names = None):
    ...
    result_file = os.path.join(FIGURES_PATH, comparison_name, "rank.txt")
