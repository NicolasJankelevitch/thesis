...
from active_semi_clustering.active.pairwise_constraints.helpers import get_constraints_from_neighborhoods
            ...
            while len(neighborhoods) < k:
                ...
                for neighborhood in neighborhoods:
                    if oracle.query(farthest, neighborhood[0]):
                        self.pairwise_constraints_[0].append((farthest, neighborhood[0]))
                        neighborhood.append(farthest)
                        new_neighborhood = False
                        break
                    else:
                        self.pairwise_constraints_[1].append((farthest, neighborhood[0]))
            ...
            try:
                ...
                for neighborhood in sorted_neighborhoods:
                    if oracle.query(i, neighborhood[0]):
                        self.pairwise_constraints_[0].append((i, neighborhood[0]))
                        neighborhood.append(i)
                        break
                    else:
                        self.pairwise_constraints_[1].append((i, neighborhood[0]))

                neighborhoods_union.add(i)
                remaining.remove(i)

            except (MaximumQueriesExceeded, ValueError):
                break
