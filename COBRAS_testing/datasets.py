...
class Dataset():
    ...
    def interesting_2d_datasets():
        return [name[0:-4] for name in os.listdir(os.path.join(DATASET_PATH, SIMPLE_DATA_DIR))]
    ...
    @classmethod
    def datasets(cls, preprocessed = True):
        if preprocessed:
            return map(lambda x: Dataset(x), cls.get_dataset_names())
        else:
            return [Dataset(x) for x in cls.get_coprf_dataset_names()]
