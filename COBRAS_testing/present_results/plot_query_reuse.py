        ...
        for dataset_name in os.listdir(test_path):
            ...
            for f in os.listdir(dataset_folder):
                ...
                with open(file) as d:
                    ...
                    unique_constraints = set(reused_constraints)
                total_dataset_reuses = total_dataset_reuses + len(reused_constraints)
                total_dataset_unique = total_dataset_unique + len(unique_constraints)

            test_reuses = test_reuses + total_dataset_reuses
            test_uniques = test_uniques + total_dataset_unique

        reuse_list.append(test_reuses)
        unique_list.append(test_uniques)
    plot_reuse(comparison_name, test_names, reuse_list, unique_list)


def plot_reuse(comparison_name, test_names, reuse_list, unique_list):
    x = np.arange(len(test_names))
    fig = plt.figure()
    ax = fig.add_axes([0, 0, 1, 1])
    ax.bar(x + 0.00, reuse_list, label="#constraints reused", width=0.25)
    ax.bar(x + 0.25, unique_list, label="#unique reused constraints", width=0.25)
    ax.set_xticks(x)
